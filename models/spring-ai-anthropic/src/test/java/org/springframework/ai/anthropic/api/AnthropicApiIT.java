...
public class AnthropicApiIT {
	...
	org.springframework.ai.anthropic.api.AnthropicApi anthropicApi = new org.springframework.ai.anthropic.api.AnthropicApi(System.getenv("Anthropic_API_KEY"));
	...
	void chatCompletionStream() {
		ChatCompletionMessage chatCompletionMessage = new ChatCompletionMessage("Hello world", Role.USER);
		Flux<ChatCompletionChunk> response = anthropicApi.chatCompletionStream(
	...
	void embeddings() {
		ResponseEntity<EmbeddingList<Embedding>> response = anthropicApi
			.embeddings(new AnthropicApi.EmbeddingRequest<String>("Hello world"));
