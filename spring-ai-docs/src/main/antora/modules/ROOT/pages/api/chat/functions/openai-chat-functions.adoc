...
//
// You can programmatically register functions with the `AnthropicChatClient` using the `AnthropicChatOptions#withFunctionCallbacks`:
//
// [source,java]
// ----
//
// AnthropicApi openaiApi = new AnthropicApi(apiKey);
//
// var defaultOptions = AnthropicChatOptions.builder()
// 	.withFunctionCallbacks(List.of(new FunctionCallbackWrapper<>(
// 		"CurrentWeather", // name
// 		"Get the weather in location", // function description
// 		new MockWeatherService()))) // function code
// 	.build();
//
// AnthropicChatClient chatClient = new AnthropicChatClient(openaiApi, defaultOptions);
//
// UserMessage userMessage = new UserMessage("What's the weather like in San Francisco, Tokyo, and Paris?");
//
// ChatResponse response = chatClient.call(new Prompt(List.of(userMessage),
// 		AnthropicChatOptions.builder().withFunction("CurrentWeather").build())); // Enable the function
// ----
//
// NOTE: Functions are registered when AnthropicChatClient is created, by you must enable in the Prompt the functions to be used in the request.
