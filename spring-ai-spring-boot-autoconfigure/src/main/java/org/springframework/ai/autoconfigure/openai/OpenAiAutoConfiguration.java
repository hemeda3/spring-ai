...
public class OpenAiAutoConfiguration {
	...
	@ConditionalOnProperty(prefix = OpenAiChatProperties.CONFIG_PREFIX, name = "enabled", havingValue = "true",
			matchIfMissing = true)
	public AnthropicChatClient openAiChatClient(OpenAiConnectionProperties commonProperties,
		...
		return new AnthropicChatClient(openAiApi, chatProperties.getOptions(), functionCallbackContext, retryTemplate);
	}
	...
	@ConditionalOnProperty(prefix = OpenAiEmbeddingProperties.CONFIG_PREFIX, name = "enabled", havingValue = "true",
			matchIfMissing = true)
	public AnthropicEmbeddingClient openAiEmbeddingClient(OpenAiConnectionProperties commonProperties,
		...
		return new AnthropicEmbeddingClient(openAiApi, embeddingProperties.getMetadataMode(),
				embeddingProperties.getOptions(), retryTemplate);
	}

	private AnthropicApi openAiApi(String baseUrl, String commonBaseUrl, String apiKey, String commonApiKey,
		...
		return new AnthropicApi(resolvedBaseUrl, resolvedApiKey, restClientBuilder, responseErrorHandler);
	}
	...
	@ConditionalOnProperty(prefix = OpenAiImageProperties.CONFIG_PREFIX, name = "enabled", havingValue = "true",
			matchIfMissing = true)
	public AnthropicImageClient openAiImageClient(OpenAiConnectionProperties commonProperties,
		...
		var openAiImageApi = new AnthropicImageApi(baseUrl, apiKey, restClientBuilder, responseErrorHandler);

		return new AnthropicImageClient(openAiImageApi, imageProperties.getOptions(), retryTemplate);
	}
	...
	@Bean
	@ConditionalOnMissingBean
	public AnthropicAudioSpeechClient openAiAudioSpeechClient(OpenAiConnectionProperties commonProperties,
		...
		AnthropicAudioSpeechClient openAiSpeechClient = new AnthropicAudioSpeechClient(openAiAudioApi,
